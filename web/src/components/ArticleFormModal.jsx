...
export default function ArticleFormModal({ articleId, onClose, onSaved }) {
  ...
  const handleSubmit = async () => {
    ...
    if (!form.summary.trim()) { setError("Vui lòng nhập tóm tắt ngắn"); return }
    if (!form.body.trim()) { setError("Vui lòng nhập nội dung chi tiết bài viết"); return }
    if (!form.imageUrl.trim()) { setError("Vui lòng tải lên ảnh bìa bài viết"); return }
    const sortVal = parseInt(form.sortOrder, 10)
    if (isNaN(sortVal) || sortVal <= 0) { setError("Thứ tự hiển thị phải lớn hơn 0"); return }
  ...
  const labelStyle = "block text-[11px] font-bold text-[#8E707E] mb-1.5 tracking-widest uppercase ml-1"
  const inputStyle = "w-full px-4 py-3 bg-[#FFF0F5] border-none rounded-xl outline-none text-[#5C4550] placeholder-[#D1B9C5] focus:ring-2 focus:ring-pink-200 transition-all font-medium text-sm"
  ...
  return createPortal(
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fadeIn">
      <div className="bg-white w-full max-w-[700px] max-h-[90vh] rounded-[2rem] shadow-2xl overflow-hidden flex flex-col relative animate-scaleIn">
        ...
        {/* HEADER + TABS */}
        <div className="p-6 pb-0 border-b border-pink-50">
          ...
          {articleId && (
            <div className="flex gap-1">
              <button
                ...
                className={`flex items-center gap-1.5 px-4 py-2.5 text-sm font-bold rounded-t-xl transition-all ${
                  activeTab === "content" ? "bg-pink-500 text-white" : "text-[#8E707E] hover:bg-pink-50"
                }`}
              >
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-4 py-2.5 text-sm font-bold rounded-t-xl transition-all ${
                  activeTab === "translations" ? "bg-pink-500 text-white" : "text-[#8E707E] hover:bg-pink-50"
                }`}
              >
                <Languages size={14} /> Bản dịch
                {translatedLangs.length > 0 && (
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ${
                    activeTab === "translations" ? "bg-white/30 text-white" : "bg-pink-100 text-pink-600"
                  }`}>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {translatedLangs.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-gray-400">
                ...
                <p className="text-sm font-medium">Chưa có bản dịch nào</p>
                <p className="text-xs mt-1">Hệ thống sẽ tự động dịch sau khi lưu bài viết.</p>
              </div>
            ) : (
                ...
                return (
                  <div key={lang} className="border border-pink-100 rounded-2xl overflow-hidden">
                    ...
                    <div className="p-4 space-y-3">
                      <div>
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Tiêu đề</p>
                        <p className="text-sm font-semibold text-gray-800">
                          {content.title || <span className="italic text-gray-300">Chưa có</span>}
                        </p>
                      </div>
                      <div>
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Tóm tắt</p>
                        <p className="text-sm text-gray-600">
                          {content.summary || <span className="italic text-gray-300">Chưa có</span>}
                        </p>
                      </div>
                      <div>
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Nội dung</p>
                        <p className="text-xs text-gray-500 whitespace-pre-wrap leading-relaxed line-clamp-4">
                          {content.body || <span className="italic text-gray-300">Chưa có</span>}
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button type="button" onClick={() => handleInputChange("type", "tip")}
                    className={`py-3 rounded-xl text-sm font-bold transition-all ${form.type === "tip" ? "bg-pink-500 text-white shadow-md shadow-pink-200" : "bg-[#FFF0F5] text-[#8E707E] hover:bg-pink-100"}`}>
                    Mẹo Du Lịch (Tip)
                  </button>
                  <button type="button" onClick={() => handleInputChange("type", "news")}
                    className={`py-3 rounded-xl text-sm font-bold transition-all ${form.type === "news" ? "bg-pink-500 text-white shadow-md shadow-pink-200" : "bg-[#FFF0F5] text-[#8E707E] hover:bg-pink-100"}`}>
                    Tin Tức (News)
                  </button>
                </div>
              </div>
              <div>
                ...
                <input type="number" min="1" value={form.sortOrder}
                  ...
                  className={inputStyle} placeholder="1 (Độ ưu tiên cao nhất)" />
              </div>
            </div>
            ...
            <div>
              ...
              <input type="text" value={form.title}
                ...
                className={inputStyle} placeholder="Nhập tiêu đề hấp dẫn..." />
            </div>
            ...
            <div>
              ...
              <textarea rows="2" value={form.summary}
                ...
                className={`${inputStyle} resize-none`} placeholder="Tóm tắt ngắn gọn hiển thị trên danh sách..." />
            </div>
            ...
            <div>
              ...
              <textarea rows="6" value={form.body}
                ...
                className={`${inputStyle} font-mono text-xs`} placeholder="Nhập nội dung đầy đủ của bài viết..." />
            </div>
              ...
              <div className="mt-1 flex flex-col md:flex-row gap-4 items-start">
                {form.imageUrl ? (
                  <div className="relative w-full md:w-48 h-32 rounded-2xl overflow-hidden border border-pink-100 group shadow-sm">
                    <img src={form.imageUrl} alt="Cover" className="w-full h-full object-cover group-hover:scale-105 transition-transform" />
                    <button type="button" onClick={() => handleInputChange("imageUrl", "")}
                  ...
                  <label className="flex-1 w-full flex flex-col items-center justify-center py-8 border-2 border-dashed border-[#E9C3D5] hover:border-pink-400 bg-[#FFF5F8]/40 hover:bg-[#FFF5F8]/80 rounded-2xl cursor-pointer transition-all group">
                    {uploading ? <Loader2 className="animate-spin text-pink-500 mb-2" size={24} />
            ...
            <div className="flex items-center justify-between p-4 bg-pink-50/50 rounded-2xl border border-pink-100/50">
              ...
              <button type="button" onClick={() => handleInputChange("isActive", !form.isActive)}
                className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${form.isActive ? "bg-pink-500" : "bg-gray-200"}`}>
        ...
        <div className="p-6 border-t border-pink-50 flex items-center justify-end gap-3 bg-gray-50/50">
          <button onClick={onClose} className="px-5 py-3 text-[#8E707E] font-bold text-sm hover:underline" disabled={loading}>
            Đóng
          </button>
          {activeTab === "content" && (
            <button onClick={handleSubmit} disabled={loading || uploading || fetching}
